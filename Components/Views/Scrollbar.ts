...
export default class Scrollbar {
   ...
   render(ItemsLength: number, capacity: number) {
      ...
      //fill cursor
      ctx.fillStyle = 'red'; //hack fill;
      ctx.fillRect(
         this.cursor.left, this.container.top + this.cursor.top,
         this.cursor.width, this.cursor.length
      );
   ...
   scroll(delta: number) {
      ...
      const newTop = (this.cursor.index * 5); //lineHeight);
