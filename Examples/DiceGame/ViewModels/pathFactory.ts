import { Location, Path2D } from '../deps.ts'
...
export function buildRightScore(location: Location, size: { height: number, width: number }) {
...
export function buildLeftScore(location: Location, size: { height: number, width: number }) {
   const { left, right, top, bottom, width, height, radius } = getPathGeometry(location, size)
...
const getPathGeometry = (location: Location, size: { height: number, width: number }, radius = 10) => {
